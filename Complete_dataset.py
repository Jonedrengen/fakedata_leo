...
def Generate_complete_data(record_amount):
    ...
    for i in range(record_amount):
        ...
        if elapsed_time >= update_time:
            update_time += 0.15
            print(f'generated {i} records of {record_amount} desired')

        #generating an ID for each datafile
        ConsensusID = GenerateUniqueConsensusID()
        SequencedSampleID= GenerateUniqueSequencedSampleID()
        NextcladeResultID = GenerateUniqueNextcladeResultID()
        SampleID = GenerateUniqueSampleID()
        BatchID = GenerateUniqueBatchID()
        PangolinResultID = GenerateUniquePangolinResultID()
...
######################## Consensus_data ########################
        ...
        #AmbiguousSites
        # If AmbiguousSites over 5, then NcountQC = fail (not implemented)
        ...
        UnaliasedPango = None
        Consensus_Nextclade_essentials = Variant_connections.sample(n=1, weights=Variant_connections_weights).iloc[0]
        WhoVariant = Consensus_Nextclade_essentials['WhoVariant']
        LineageOfInterest = Consensus_Nextclade_essentials['LineagesOfInterest']
        UnaliasedPango = Consensus_Nextclade_essentials['UnaliasedPango']
...
######################## NextcladeResult_data ########################
        ...
        clade = Consensus_Nextclade_essentials['clade']
        Nextclade_pango = Consensus_Nextclade_essentials['Nextclade_pango']
...
######################## RECORDS ########################
        ...
        Consensus_record = {
            ...
            "NCount": NCount, #above 3k = not passed (not implemented)
            "AmbiguousSites": AmbiguousSites, # over 5, then NcountQC = fail (not implemented)
            "NwAmb": NwAmb,
            "NCountQC": NCountQC,
        ...
        NextcladeResult_data.append(NextcladeResultID_record)
...
if __name__ == '__main__':
    ...
    headers = [
        ...
        #Sample_data headers
        "SampleID", "Host", "Ct", "DateSampling", "CurrentConsensusID", "TimestampCreated", "TimestampUpdated", "SampleDateTime",
        #Batch_data Headers
        "BatchID", "BatchDate", "Platform", "BatchSource", "TimestampCreated", "TimestampUpdated",
        #SequencedSample_data headers
        ...
        #PangolinResult_data headers
        "PangolinResultID", "lineage", "version", "pangolin_version", "scorpio_version", "constellation_version", 
