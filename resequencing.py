...
for i, batch in batches_to_reuse.iterrows():
    ...
    for i, sequenced_sample in samples_in_batch.iterrows():
        ...
        orig_consensus = Consensus_data[Consensus_data["QcVariantConsensusID"] == original_consensus_id]
        orig_consensus = orig_consensus.iloc[0]
        #print(orig_consensus)

        orig_nextclade_id = orig_consensus["CurrentResultsNextcladeID"]
        #print(orig_nextclade_id)
        orig_nextclade = NextcladeResult_data[NextcladeResult_data["ResultsNextcladeID"] == orig_nextclade_id]
        orig_nextclade = orig_nextclade.iloc[0]

        orig_pangolin_id = orig_consensus["CurrentPangolinID"]
        orig_pangolin = PangolinResult_data[PangolinResult_data["PangolinID"] == orig_pangolin_id]
        orig_pangolin = orig_pangolin.iloc[0]
        
        new_consensus_record = orig_consensus.copy()
        new_consensus_record = new_consensus_record.to_dict()  # Convert Series to dict
        ...
        new_nextclade = new_nextclade.to_dict()
        ...
        new_pangolin = new_pangolin.to_dict()
